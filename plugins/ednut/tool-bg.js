module.exports = [
  {
    ...
    execute: async (m, { ednut, uploadImage }) => {
      try {
        ...
        const api = `https://archive.lick.eu.org/api/tools/removebg?url=${encodeURIComponent(imageUrl)}`;
        const res = await fetch(api);
        const json = await res.json();

        if (!json?.result) return m.reply("❌ Could not remove background.");
        await ednut.sendMessage(m.chat, { image: { url: json.result }, caption: global.footer }, { quoted: m });
      } catch (err) {
        global.log("ERROR", `removebg: ${err.message}`);
        m.reply("❌ Error removing background.");
      }
    },
  },
  {
